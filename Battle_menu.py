...
buttons = [
    [classic, C_button, blue, violet_blue, 200, 600, 100, 50, 'classic'],
    [new, N_button, red, dar_red, 400, 600, 100, 50, 'salvo']
]


def button_hover(button_x, button_w, button_y, button_h, disply, color, rect, text, hover_color, action):
    ...
    if button_x + button_w > mouse[0] > button_x and button_y + (button_h / 2) > mouse[1] > button_y:
        ...
        if click[0] == 1 and action == 'classic':
            game.gameon()
        elif click[0] == 1 and action == 'salvo':
            game.gameon()
...
def menu_screen():
    ...
    while menu:
        ...
        for text, rect, color, hover, B_x, B_y, B_w, B_h, action in buttons:
            button_hover(B_x, B_h, B_y, B_w, Display, color, rect, text, hover, action)
